...
if __name__ == '__main__':
    hmm = HMM(**input_data['hmm'])

    observations = hmm.generate(input_data['t'])
    print(f'Algoritmul de generare de observatii:\n'
          f'Secventa: {", ".join(["{0:.10f}".format(observation) for observation in observations])}\n')

    forward_probability = hmm.forward(input_data['test'])
    print(f'Algoritmul forward:\n'
          f'Probabilitate: {forward_probability:.20f}\n')

    (viterbi_probability, viterbi_sequence) = hmm.viterbi(input_data['test'])
    print(f'Algoritmul Viterbi:\n'
          f'Probabilitate: {viterbi_probability:.20f}\n'
          f'Secventa: {" ".join([str(x) for x in viterbi_sequence])}\n')

    hmm_baum_welch = HMM(**input_data['hmm_baum_welch'])
    train_results = hmm_baum_welch.baum_welch(input_data['test'], input_data['hmm_baum_welch_steps'])
